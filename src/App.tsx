...
const App: React.FC = () => {
  ...
  const handleCheckoutSubmit = async (formData: OrderForm) => {
    ...
    try {
        ...
        // GAS returns a redirect, which fetch follows. 
        // If "Who has access" is ANYONE, this works.
        // If not, this throws a CORS error.
        
        // We might get an opaque response depending on how GAS redirects, 
        // but typically with text/plain body, we might get a readable response.
        
        const resultText = await response.text();
        
        // Try parsing JSON if possible, otherwise assume success if no error thrown
        try {
          ...
          // If response isn't JSON, it might be the HTML redirect page.
          // This usually means it worked if we didn't get a 401/403.
